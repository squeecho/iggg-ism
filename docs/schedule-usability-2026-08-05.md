...
- 작업 브랜치: `fix/chart-single-row-20260807`
- 범위: 1~5차 차트 막대를 38px 고정 행의 동일한 세로 중앙에 표시하고 화면·PNG·PDF 출력 계약을 일치시킨다.
- 안전 경계: 특별 날짜 편집과 일정 데이터 정본은 변경하지 않는다. 합성 상태와 차단된 네트워크만 사용하며 Calendar API, Firebase, 고객 데이터, 전자계약, 메시징, SMBM 경로는 조회하거나 변경하지 않는다.

### 원문과 현재 구현 대조

| 근거 | 직접 확인한 사실 | 판정 범위 |
|---|---|---|
| Production 기준 `rG()` | 다차수이면 행 높이를 56px로 바꾸고 홀수/짝수에 따라 `laneTop=4/30px`을 inline style로 설정한다. | 차수 수에 따라 행이 커지고 막대가 두 레인으로 갈라지는 직접 원인이다. |
| 공통 `.dg`와 `.bar` CSS | 날짜 셀은 38px이고 막대 기본값은 `top:50%; transform:translateY(-50%)`이다. | 단일 중앙 행 정본은 이미 있으며 `rG()`의 inline override가 이를 무효화한다. |
| 현재 실제 Chromium 기준선 | 합성 1~5차 화면에서 단일 차수는 38px, 다차수 행은 56px이고 홀수/짝수 막대가 서로 다른 y 중심에 표시된다. | 정적 코드뿐 아니라 실제 렌더에서도 회귀를 재현했다. |
| `doIMG()`와 `doPDF()` | 두 함수 모두 화면의 동일한 `#pa/#gt` DOM을 `html2canvas`로 캡처한다. | 출력 전용 레이아웃은 없으며 차트 DOM 수정이 화면·PNG·PDF에 함께 적용된다. |
| phase와 특별 날짜 이벤트 | phase drag/resize는 x 좌표와 `data-task-id/data-phase`만 사용하고, 특별 날짜는 별도 pointer/card handler를 사용한다. | 세로 배치만 바꾸고 날짜·저장·undo/autosave 및 특별 날짜 계약을 보존할 수 있다. |

### 경쟁 가설과 판정

| 가설 | 판정 | 근거 |
|---|---|---|
| `rowHeight=56`과 홀짝 `laneTop` inline 계산이 2단 배치의 원인이다. | 확인 | 실제 수식, DOM inline style, 38/56px 렌더 측정이 일치한다. |
| 공통 `.bar` CSS가 막대를 위·아래로 나눈다. | 기각 | 공통 CSS는 이미 모든 막대를 50% 중앙에 두며 inline `top`과 `transform:none`에 덮인다. |
| 차수 저장 형식이나 자동배치가 여러 행을 생성한다. | 기각 | 한 공종은 하나의 `<tr>`만 만들고 phase 정본은 날짜 범위만 반환한다. |
| PNG/PDF가 별도 렌더러라 화면 수정만으로는 출력이 달라진다. | 기각 | 두 출력 모두 현재 화면과 같은 차트 DOM을 캡처한다. |
