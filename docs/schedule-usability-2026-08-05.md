...
| W2-04 | 통합 일정 3차 수정이 1차를 덮는 기존 결함 제거 | deviant | P0 | `index.html:7584-7588` | 공통 phase setter로 도달 불가능한 분기를 제거하고 1~5차 저장 회귀 테스트를 둔다. |
| W2-05 | 사용자 공종 CRUD 및 저장 | missing | P0 | `index.html:3255`, `4618`, `6035`, `8586` | 안전정수 ID와 `custom:true`를 저장한다. 기본 공종 삭제는 handler에서 거부하고 동적 공종 목록을 전 경로에 포함한다. |
| W2-06 | 특별 날짜 자동/수동 토글 | partial | P0 | `index.html:1644-1654`, `3581-3944` | 명시적 `dateMode`를 도입한다. 직접 선택은 수동, 자동 복귀는 즉시 재계산, 기간 밖 수동 날짜는 경고만 표시한다. |
| W2-07 | 기존 저장 데이터 무손실 정규화 | partial | P0 | `index.html:1865-1915`, `3946-3960`, `4398`, `5758-5928` | 모든 local/cloud/hash ingress에서 하나의 정규화 함수를 호출한다. 기존 2·3차와 특별 날짜 의미를 보존한다. |
...
| W3-03 | 1~5차 막대 겹침 방지 | deviant | P0 | `index.html:608`, `3749-3787`, `327-344` | 행 높이를 phase 수에 맞춰 계산한다. 모바일/데스크톱에서 라벨·인접 행과 겹치지 않아야 한다. |
| QA-01 | 기존 데이터 정규화와 저장/재로드 자동 테스트 | missing | P0 | 테스트 인프라 없음 | Node 기본 test runner로 순수 helper의 1~5차, legacy, 사용자 공종, 날짜 모드를 검증한다. |
| QA-02 | 격리 브라우저 실제 UI 검증 | missing | P0 | 외부 SDK/운영 API가 기본 로드됨 | 임시 브라우저 프로필에서 외부 mutation을 차단하고 합성 localStorage만 사용한다. console/page error 및 mutation 0건을 기록한다. |
